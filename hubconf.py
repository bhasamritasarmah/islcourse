...
def _test_network_advanced(dataloader, config, model, loss_fn):
    size = len(dataloader.dataset)
    num_batches = len(dataloader)
    model.eval()
    test_loss, correct = 0, 0
...
def test_model_advanced(dataloader, config, model, loss_fn, epochs = 10):
  for e in range(epochs):
    print(f"Epochs {e+1}: ")
    _test_network_advanced(dataloader, config, model, loss_fn)
